...
const CreateTicketModal = forwardRef<CreateTicketModalRef, CreateTicketModalProps>(
  function CreateTicketModal({ services, statuses, ticketTypes }, ref) {
    ...
    const inputClass = 'w-full rounded-md border border-border/60 bg-surface px-3 py-2 text-sm text-slate-200 outline-none transition placeholder:text-slate-600 focus:border-accent/60';
        ...
        {isOpen && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-lg rounded-xl border border-border/60 bg-surface2 shadow-2xl shadow-black/30"
              onClick={(e) => e.stopPropagation()}
            >
          {/* Header */}
          <div className="flex items-center justify-between border-b border-border/40 px-5 py-3">
            <h2 className="text-sm font-semibold text-slate-200">Novo ticket</h2>
            <button onClick={() => setIsOpen(false)} className="text-slate-500 transition hover:text-slate-300">
              <X size={16} />
            </button>
          </div>
          ...
          <form className="space-y-4 p-5" onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                <label className="mb-1 block text-[11px] font-medium text-slate-500">Serviço</label>
                <select value={serviceId} onChange={(e) => setServiceId(e.target.value)} className={inputClass}>
                  {services.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
              </div>
              <div>
                <label className="mb-1 block text-[11px] font-medium text-slate-500">Prioridade</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="mb-1 block text-[11px] font-medium text-slate-500">Data limite</label>
                <input
                  type="date"
                  value={dueDate}
            ...
            <div className="flex items-center gap-2 border-t border-border/40 pt-4">
              <button
                ...
                className="rounded-md bg-accent px-4 py-2 text-xs font-semibold text-white transition hover:bg-blue-500 disabled:opacity-50"
              >
              ...
              <button
                type="button"
                onClick={() => setIsOpen(false)}
                className="rounded-md px-4 py-2 text-xs text-slate-400 transition hover:bg-input/40 hover:text-slate-200"
              >
                Cancelar
              </button>
            </div>
